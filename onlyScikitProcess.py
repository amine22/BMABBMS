...
twenty_train = fetch_20newsgroups(subset='train', categories=cats, shuffle=True, random_state=42)#, remove=('headers', 'footers', 'quotes'))
...
#wordmatch(data)
vectorizer = TfidfVectorizer( stop_words="english", use_idf=True,norm='l2',max_df=0.5,max_features=None,ngram_range=(1,2), tokenizer=tokenize) #min_df=1,
X_train_tfidf = vectorizer.fit_transform(data)

''' NAIVE BAYES '''

#clf = MultinomialNB(alpha= 1e-05)#,n_iter= 80,penalty= 'elasticnet')
''' SGDC '''
from sklearn.linear_model import SGDClassifier
clf = SGDClassifier(alpha= 1e-05,n_iter= 80,penalty= 'elasticnet')
clf.fit(X_train_tfidf, twenty_train.target)

twenty_test = fetch_20newsgroups(subset='test',categories=cats,shuffle=True, random_state=42)#, remove=('headers', 'footers', 'quotes'))
...
print "CLF saved! "
print ''
f = open('NaiveBayesVectorizer.pickle', 'wb')
